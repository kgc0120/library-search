...
class BookQueryServiceItTest extends Specification {
    ...
    def "정상상황에서 Circuit의 상태가 CLOSED이고, naver로 호출이 들어간다."() {
        given:
        def keyword = 'HTTP'
        def page = 1
        def size = 10
    ...
    def "circuit이 open되면 kakao쪽으로 요청한다."() {
        given:
        def keyword = 'HTTP'
        def page = 1
        def size = 10
        ...
        naverBookRepository.search(keyword, page, size) >> { throw new RuntimeException("네이버 API 실패") }

        when:
        def result = bookQueryService.search(keyword, page, size)

        then: "kakao 쪽으로 fallback이 발생한다."
        1 * kakaoBookRepository.search(keyword, page, size) >> kakaoResponse

        and: "circuit이 open 상태가 된다."
        def circuitBreaker = circuitBreakerRegistry.getAllCircuitBreakers().stream().findFirst().get()
