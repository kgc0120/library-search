...
public class KakaoBookRepository implements BookRepository{
        ...
        public PageResult<SearchResponse> search(String query, int page, int size) {
            KakaoBookResponse kakaoBookResponse = kakaoClient.search(query, page, size);

            List<SearchResponse> responses = kakaoBookResponse.documents().stream()
                    .map(this::createResponse)
                    .collect(Collectors.toList());
            return new PageResult<>(page, size, kakaoBookResponse.meta().totalCount(), responses);
        }
